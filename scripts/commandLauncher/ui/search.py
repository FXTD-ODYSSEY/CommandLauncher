...
class SearchWidget(utils.QWidget):
    ...
    * Search results.
    ...
    def __init__(self, parent=None):
        ...
        # NOTE 加载工具架和菜单 | 这些操作存在异步，放在多线程里面执行会出错或者获取不到
    ...
    def eventFilter(self, receiver, event):
        ...
        if hasattr(event, "type") and event.type() == utils.QEvent.KeyRelease:
            # NOTE 敲击 Tab 键
            if event.key() == utils.Qt.Key_Tab and not self.isVisible() and self.manager.setting.setting_data["Tab_CB"]:
    ...
    def process(self, num):
        ...
        if self.search.filter_mode:
            matches = filter(lambda x: x["category"]
                             == self.search.filter_mode, matches)

        # NOTE 默认只显示 100 个结果提高搜索效率
        if self.search.display_num:
            matches = matches[:self.search.display_num] if len(
                matches) > self.search.display_num else matches
    ...
    def show(self):
        pos = utils.QCursor.pos() - utils.mayaWindow().pos() - utils.QPoint(32, 37)
        self.move(pos)
        self.search.setFocus()
        self.search.selected = 0
        # NOTE 如果文字留存则显示上次输入的
...
class SearchEdit(utils.QLineEdit):
    ...
    def defaultPressAction(self,event):
        # NOTE 过滤 modifier 输入的事件(除了 lineedit 本身的事件)
        defaultKeySequence = [
            # "Right"        ,
            # "Home"         ,
            # "End"          ,
        ...
        KeySequence = utils.QKeySequence(key+modifiers)
        for key in defaultKeySequence:
            if KeySequence == utils.QKeySequence(key):
    ...
    def keyPressEvent(self, event):
        ...
        # NOTE 开启 menu 过滤模式
        if KeySequence == utils.QKeySequence("Ctrl+Q"):
        ...
        if self.count < 1:
            return self.defaultPressAction(event)
   
        # NOTE 快捷键显示切换
        if key == utils.Qt.Key_Control:
            return [item.shortcut.setText(item.shortcut.text().replace("alt", "ctrl")) for _, [item, _] in self.shortcut.items()]
        elif key == utils.Qt.Key_Alt:
            return [item.shortcut.setText(item.shortcut.text().replace("ctrl", "alt")) for _, [item, _] in self.shortcut.items()]
    ...
    def jumpToPins(self, num=0):
        ...
        if num:
            btn_list = manager.RB_dict.values()
            if len(btn_list) <= num-1:
                return
            btn = btn_list[num-1]
            btn.toggle()
            pins_list = manager.setActive(btn)

            self.parent.typing()
            if pins_list and not self.parent.results.isVisible():
                widget = self.parent.results.widget
                widget.populate(pins_list)
                self.parent.results.show(len(pins_list))
                self.pressDownKey()
        else:
            if not hasattr(manager, "group"):
                return
            btn = manager.group.checkedButton()
            search = self.parent.search.text()
            if len(search) < 4:
